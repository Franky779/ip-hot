...
import { getCachedResearchHtmlDocument, prepareResearchHtmlDocument, researchHtmlResponseHeaders, setCachedResearchHtmlDocument } from '@/lib/research-html'
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Check in-memory cache first to avoid hitting DB for large HTML
  const cached = getCachedResearchHtmlDocument(id)
  if (cached) return new Response(cached, { headers: researchHtmlResponseHeaders(new URL(request.url).origin) })
  ...
  const document = prepareResearchHtmlDocument(result.data.markdown_content)
  setCachedResearchHtmlDocument(id, document)
  return new Response(document, { headers: researchHtmlResponseHeaders(new URL(request.url).origin) })
}