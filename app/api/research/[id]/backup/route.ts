...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const result = await client.from('research_reports').select('id, slug, category, title, published_at, markdown_content, content_format').eq('id', id).maybeSingle()
  if (result.error) return Response.json({ error: result.error.message }, { status: 500 })
  if (!result.data) return Response.json({ error: '报告不存在' }, { status: 404 })
  const report = result.data as Pick<ResearchReport, 'slug' | 'category' | 'title' | 'published_at' | 'markdown_content' | 'content_format'>
  try {
    const backup = await backupResearchToGithub(report)
