...
const MAX_FILES = 50
const MAX_BYTES = 20 * 1024 * 1024 // 单文件 20MB

// 管理员：批量上传「IP快闪/美陈方案」图片，存图并更新 event_plans 元数据
// POST /api/admin/ipbrand/upload-event-plan  multipart: id, files[]
export async function POST(request: Request) {
  ...
  try {
    ...
    for (const file of files) {
      ...
      const fileName = uniqueFileName('eventplan', file.name)
    ...
    const key = String(id)
    admin.event_plans[key] = [...(admin.event_plans[key] || []), ...saved]
