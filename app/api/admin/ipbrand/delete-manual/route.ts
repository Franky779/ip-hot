...
// 管理员：删除某 IP 的一张品牌手册图片（删磁盘文件 + 移除元数据）
// POST /api/admin/ipbrand/delete-manual  body: { id: number, name: string }
export async function POST(request: Request) {
  ...
  try {
    ...
    const key = String(id)
    const list = admin.manuals[key] || []
    const item = list.find(x => x.name === name)
    if (!item) {
      return NextResponse.json({ error: '手册图片不存在' }, { status: 404 })
    }
    ...
    admin.manuals[key] = list.filter(x => x.name !== name)
    if (admin.manuals[key].length === 0) delete admin.manuals[key]
