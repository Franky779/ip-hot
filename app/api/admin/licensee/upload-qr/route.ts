...
import { isLicenseeImage } from '@/lib/licensee-types'
...
export async function POST(request: Request) {
  ...
  try {
    const folder = licenseeFolderName(id, name)
    const dir = join(getLicenseeImageDir(), folder)
    mkdirSync(dir, { recursive: true })
    const urls: string[] = []
    for (const file of files) {
      if (!isLicenseeImage(file.name)) return NextResponse.json({ error: `不支持的文件类型: ${file.name}` }, { status: 400 })
      if (file.size > MAX_BYTES) return NextResponse.json({ error: `${file.name} 超过 10MB` }, { status: 400 })
      const filename = uniqueLicenseeFilename('qr', file.name)
      writeFileSync(join(dir, filename), Buffer.from(await file.arrayBuffer()))
      urls.push(licenseeImagePath(folder, filename))
    }
    const admin = loadLicenseeAdmin()
    const current = admin.edits[String(id)] || {}
    admin.edits[String(id)] = { ...current, qr_images: [...(current.qr_images || []), ...urls] }
    saveLicenseeAdmin(admin)
    return NextResponse.json({ ok: true, files: urls.map(local => ({ local })) })
  } catch (e) {
