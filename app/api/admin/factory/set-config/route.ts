...
import { mergeFactoryRecords, type FactoryRecord } from '@/lib/factory-types'
...
export async function POST(request: Request) {
  ...
  if (Array.isArray(body.custom_categories)) {
    ...
    if (removed.length) {
      const records = JSON.parse(readFileSync(join(process.cwd(), 'public', 'factory', 'factories.json'), 'utf8')) as FactoryRecord[]
      const allRecords = mergeFactoryRecords(records, admin)
      ...
      if (related.length) return NextResponse.json({ error: `无法删除品类“${related[0].category}”：仍有供应链“${related[0].name}”使用该品类，请先调整关联信息` }, { status: 409 })
    }
