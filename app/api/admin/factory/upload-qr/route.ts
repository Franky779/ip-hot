...
import { isFactoryImage } from '@/lib/factory-types'
...
export async function POST(request: Request) {
  ...
  try {
    const folder = factoryFolderName(id, name)
    const dir = join(getFactoryImageDir(), folder)
    mkdirSync(dir, { recursive: true })
    const urls: string[] = []
    for (const file of files) {
      if (!isFactoryImage(file.name)) return NextResponse.json({ error: `不支持的文件类型: ${file.name}` }, { status: 400 })
      ...
      urls.push(`${factoryImagePath(folder, filename)}`)
    }
