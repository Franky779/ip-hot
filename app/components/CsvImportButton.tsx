          ...
          <div className="csv-preview-modal" onClick={(e) => e.stopPropagation()}>
            ...
            {(importing || done || error) && (
              <div style={{ padding: '0.75rem 1.25rem', borderBottom: '1px solid var(--border-light)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
                  <div style={{
                    ...
                    <div style={{
                      ...
                      background: error ? '#e53e3e' : done ? '#2d8a4e' : 'var(--accent, #2563eb)',
                      width: error ? '100%' : `${progress}%`,
                      transition: 'width 0.25s ease'
                    }} />
                  </div>
                  <span style={{ fontSize: '0.8125rem', fontWeight: 600, minWidth: '2.8rem', textAlign: 'right', color: error ? '#e53e3e' : done ? '#2d8a4e' : 'var(--text)' }}>
                    {error ? '失败' : `${progress}%`}
                  </span>
                </div>
                {importing && (
                ...
                {error && (
                  <p style={{ margin: '0.35rem 0 0', fontSize: '0.75rem', color: '#e53e3e' }}>
                    {error}
                  </p>
                )}
              </div>
            ...
            <div className="csv-preview-actions">
              ...
              {done ? (
                ...
                <button className="talks-admin-save-btn" onClick={close}>
                  关闭
                </button>
              ) : (
