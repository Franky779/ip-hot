...
type BackupInput = { category: ResearchCategory; slug: string; title: string; published_at: string; markdown_content: string }
...
export async function backupResearchToGithub(input: BackupInput): Promise<{ path: string }> {
  ...
  const path = githubResearchPath(input.category, input.slug)
