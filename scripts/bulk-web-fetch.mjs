...
// 读取源列表
const sourcesList = JSON.parse(readFileSync(resolve(process.cwd(), 'scripts/sources-full.json'), 'utf-8'))
// 过滤：WEB源，排除已跑通的、需CDP的、需登录的
const doneIds = new Set(['sanwenyu-web', 'gamersky-acg', '17173-acg'])
const webSources = sourcesList.filter(s =>
  s.category === 'WEB' &&
  !s.needsLocalCdp &&
  !s.loginRequired &&
  !doneIds.has(s.id) &&
  s.scrapeConfig &&
  s.scrapeConfig.itemSelector
)
log(`待测试WEB源: ${webSources.length} 个`)
...
async function main() {
  ...
  for (const src of webSources) {
    ...
    let result
    try {
      result = await scrapeNewsList(src.name, src.url, src.scrapeConfig)
    } catch (e) {
      log(`  抓取失败: ${e.message}`)
    ...
    if (result.error) {
      ...
      results.failed.push({ ...src, error: result.error })
      continue
    }
  ...
  writeFileSync(resolve(process.cwd(), 'scripts/bulk-web-results.json'), JSON.stringify(results, null, 2), 'utf-8')
  log('结果已保存到 scripts/bulk-web-results.json')
}
